...
**Visual snapshots**: `e2e/__screenshots__` baselines are generated by the `visual` CI job (Linux container) and committed to the PR branch. Never run Playwright with `--update-snapshots` locally — macOS pixels won't match.
