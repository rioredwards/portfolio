...
DogTown is a self-hosted photo-sharing app running on my Raspberry Pi. Naturally, the theme is **dog photos** because dogs are awesome. 🐶
...
Architecturally, I treated it like a real production app: clear separation between UI and server logic, predictable data models, and careful handling of user-generated content. The result is something that feels snappy and “product-like,” even though it's running on a tiny computer in my house.
...
To keep uploads safe (and on-theme), I built **DogBot**: a small **FastAPI** service that validates images before they’re accepted. It returns signals like `isDog` and `isNSFW` using **PyTorch/TorchVision** and an **OpenNSFW-style** classifier. The goal wasn’t “perfect AI”—it was a pragmatic safety layer that makes moderation easier and prevents obvious bad uploads from ever landing in the database.
...
DogTown also includes a lightweight profiler service built with **Hono + Node.js (TypeScript)**. It collects system and container metrics from **/proc**, **/sys**, and the **Docker socket**, then streams updates to the frontend in real time. This started as “I wonder how hard this Pi is working,” and turned into a surprisingly useful visibility tool.

I stream metrics using **Server-Sent Events (SSE)** to keep the implementation simple and efficient. Requests are authenticated, and the UI presents a dashboard-style view of CPU, memory, disk, and container health—basically a tiny observability layer tailored to this project.
...
- I built a “glowing gradient border” UI component (might release it as an npm package someday).
...
DogBot runs as a middleware service, verifying that all uploaded images are “SFW” and contain dogs.
...
I spent more time learning Docker/Docker Compose than anything else in this project. Worth it though—it also inspired a side project: a CLI utility for quickly casting commands from your history.

<LightboxImage src={ImgDogTownFzfCmd} alt="fz-cmd project image" />
...
It was “feature-complete.” So naturally, I added more features. 🤷‍♂️

- **MDX Support**: write markdown + embed components
- **Lightbox UX**: fullscreen image/video previews
- **Logo / Branding**: designed in Figma

<LightboxImage src={ImgDogTownLogoProg} alt="Logo progression" />

---

## Final Thoughts

I learned a lot from this project—Docker, DevOps, Linux, bash scripting, networking, and more. These are skills I'll benefit from for the rest of my career.

And since I mentioned AI…
...
<LightboxImage src={ImgDogTownMyPi} alt="A Raspberry Pi" />
