...
export function Sidebar() {
  return (
    <aside className="group fixed left-4 top-1/2 -translate-y-1/2 z-50 hidden md:block text-sidebar">
      <nav className="flex flex-col gap-1 [--border-radius:var(--radius-3xl)] [--padding:calc(var(--spacing)*2)] rounded-(--border-radius) bg-sidebar/80 backdrop-blur-sm p-(--padding) transition-all duration-300">
        {socialLinks.map((link, index) => (
          <Link
            ...
            className="group/item flex items-center gap-0 group-hover:gap-3 rounded-[calc(var(--border-radius)-var(--padding))] px-2 py-2 group-hover:px-3 text-sidebar-foreground transition-all duration-300 ring-ring/50 hover:bg-sidebar/50 hover:text-sidebar-primary"
